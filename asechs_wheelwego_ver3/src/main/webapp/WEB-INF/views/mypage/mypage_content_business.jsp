...
<p class="paging text-center" style="font-size: 17px; color:black;">
   ...
   <a href="${pageContext.request.contextPath}/afterLogin_mypage/showMyContentBybusinessList.do?contentPageNo=${pb.startPageOfPageGroup-1}&id=${sessionScope.memberVO.id}">
   ...
   <a href="${pageContext.request.contextPath}/afterLogin_mypage/showMyContentBybusinessList.do?contentPageNo=${i}&id=${sessionScope.memberVO.id}"">${i}</a> 
   </c:when>
   <c:otherwise>
   ${i}
   </c:otherwise>
   ...
   <a href="${pageContext.request.contextPath}/afterLogin_mypage/showMyContentBybusinessList.do?contentPageNo=${pb.endPageOfPageGroup+1}&id=${sessionScope.memberVO.id}">
    ...
    $(document).ready(function(){
       $(".updateBtn").click(function(){
    	   var contentNo=$(this).parent().parent().children(".no").text();
			   if(confirm("등록된 게시물을 수정하시겠습니까?")){
				  location.href="${pageContext.request.contextPath}/afterLogin_board/business_update_form.do?no="+contentNo;
			  }
       });// click
       $(".deleteBtn").click(function(){
          ...
          if(confirm("등록된 게시물을 삭제하시겠습니까?")){
             $.ajax({
                url:"${pageContext.request.contextPath}/afterLogin_mypage/businessDeleteInMaypage.do",
                type:"post",
